...
struct ContentView: View {
    ...
    var body: some View {
        NavigationStack {
            List(ytAPIModel.searchResult.items, id: \.id["videoId"]) { video in
                Button {
                    ytAPIModel.currentVideo = video
                } label: {
                    VideoListItem(video: video)
                }
                
            }.navigationTitle("📹 Content")
            .sheet(item: $ytAPIModel.currentVideo) { video in
             SafariView(url: URL(string: "https://youtube.com/watch?v=\(video.id["videoId"] ?? "")") ?? URL(string: "https://youtube.com")!)
             }
            
            ResultsStack(pageInfo: ytAPIModel.searchResult.pageInfo)
        }
    }
}
...
#Preview {
    ContentView(ytAPIModel: YoutubeAPIModel())
}
